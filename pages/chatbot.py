...
if "messages" not in st.session_state:
    ...
    if st.session_state['messages']==[]:
        start_string="오늘 당신의 기분은 어떠신가요?"
        st.session_state["messages"].append({"role": "assistant", "content": start_string})
        save_chat(start_string,"","")
...
def extract_keywords(prompt):
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "You are a helpful assistant. You should extract important keywords"},
...
if prompt := st.chat_input():
    ...
    save_chat(prompt, "", important_words)
